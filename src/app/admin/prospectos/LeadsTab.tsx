...
/**
 * Tab para la sub-vista "Leads (solo email)".
 *
 * Diferencia clave vs Prospectos: los leads son emails capturados sin cuenta
 * (newsletter, popups, lead magnets). No tienen login ni perfil. Si después
 * se registran formalmente, aparecen también en la tab "Prospectos" — los
 * dos pueden coexistir para el mismo email.
 *
 * El badge "✓ CLIENTE" se muestra cuando ya hay una `orders.email = lead.email`
 * con status='succeeded' — significa que ese lead se convirtió.
 */
export function LeadsTab({ rows }: { rows: LeadRow[] }) {
  ...
  const bulkDelete = useBulkDelete<number>({
    url: "/api/admin/leads/bulk-delete",
    entityLabel: { singular: "lead", plural: "leads" },
    description:
      "Los leads son emails capturados (newsletter, popups, lead magnets). Borrarlos no afecta " +
      "cuentas registradas ni órdenes. Si el mismo email se registra después, aparecerá como " +
      "prospecto/cliente normalmente. Esta acción no se puede deshacer.",
  ...
  return (
    <>
      ...
      <div className="col" style={{ gap: 0 }}>
        {filtered.map((l) => {
          const isChecked = bulk.isSelected(l.id);
          return (
            <div
              key={l.id}
              className="row"
            ...
            >
              <span style={{ width: 24, display: "flex", alignItems: "center" }}>
                <BulkCheckbox
                  checked={isChecked}
                  onChange={(c) => bulk.toggleOne(l.id, c)}
                  ariaLabel={`Seleccionar ${l.email}`}
                />
              </span>
      ...
      <BulkActionBar
        ...
        entityLabel={{ singular: "lead", plural: "leads" }}
        subtitle="LIMPIA EL BUZÓN DE NEWSLETTER · NO AFECTA CUENTAS"
        onCancel={bulk.clear}
